...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
//custno의 최대값, 최대값+1(자동생성값)을 custno input value에 입력
Class.forName("oracle.jdbc.driver.OracleDriver");
Connection conn
=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","1234");
//System.out.println(conn);
String sql="select max(custno) cn from member_tbl_02";
PreparedStatement ps=conn.prepareStatement(sql);
ResultSet rs=ps.executeQuery();
int custno=0;
if(rs.next()){
	//int custno=rs.getInt("max(custno)");
	custno=rs.getInt("cn")+1;
}
rs.close();
ps.close();
conn.close();
...
<body>
    ...
    <section>
        <h1>홈쇼핑 회원 등록</h1>
        <p>
        	<form action="regmemberProc.jsp" onsubmit="return confirm()"
        	method="post">
            <table border="1">
            <tr>
            	<td>회원번호(자동발생)</td>
            ...
            <tr>
            	<td>가입일자</td>
            	<td><input type="text" name="joindate" id="joindate" value=""></td>
            </tr>
            <tr>
